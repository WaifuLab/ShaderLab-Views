...
import {Completion, CompletionContext, CompletionResult, CompletionSource} from "autocomplete"
...
/**
 * Helper function for defining JavaScript completion sources. It returns the completable name and object
 * path for a completion context, or null if no name/property completion should happen at that position.
 * For example, when completing after `a.b.c` it will return `{path: ["a", "b"], name: "c"}`. When
 * completing after `x` it will return `{path: [], name: "x"}`. When not in a property or name, it will
 * return null if `context.explicit` is false, and `{path: [], name: ""}` otherwise.
 */
...
/**
 * Defines a [completion source]{@link CompletionSource} that completes from the given scope object
 * (for example `globalThis`). Will enter properties of the object when completing properties on a
 * directly-named path.
 */
