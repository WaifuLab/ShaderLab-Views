...
export class Context {
    ...
    translateExpr(expr, token) {
        switch (expr.type) {
            ...
            case "CHOICE":
                ...
                return opt ? this.translateInner(opt, token, 10) + "?" : expr.members.map(e => this.translateInner(e, token, prec(expr))).join(" | ");
    ...
    translateRule(name, content, top) {
        if (!top && content.type === "TOKEN") {
            ...
            this.rules[(top ? "@top " : "") + this.translateName(name)] = `${comment}{\n  ${result.join(" |\n  ")}\n}`;
        }
    }
