...
class TestSpec {
    /**
     * @param {string} name
     * @param {{prop:NodeProp,value:any}[]} props
     * @param {TestSpec[]} children
    ...
    static parse(spec) {
        ...
        function err() { throw new SyntaxError("Invalid test spec: " + spec); }
        function next() {
...
export function testTree(tree, expect, mayIgnore = defaultIgnore) {
 ...
 * @param {(type:NodeType)=>boolean} mayIgnore
 * @return {{name:string,
 *           text:string,
 *           expected:string,
 *           configStr:string,
