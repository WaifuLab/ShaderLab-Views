...
function measureRange(view: EditorView, range: SelectionRange): Piece[] {
    ...
    function piece(left: number, top: number, right: number, bottom: number) {
        return new Piece(left - base.left, top - base.top - C.Epsilon, right - left, bottom - top + C.Epsilon, "cm-selectionBackground")
    }
    ...
    function drawForLine(from: null | number, to: null | number, line: {from: number, to: number}) {
        ...
        function addSpan(from: number, fromOpen: boolean, to: number, toOpen: boolean, dir: Direction) {
            // Passing 2/-2 is a kludge to force the view to return coordinates on the proper
            // side of block widgets, since normalizing the side there, though appropriate for
            // most coordsAtPos queries, would break selection drawing.
