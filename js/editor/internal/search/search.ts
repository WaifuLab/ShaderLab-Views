...
import {EditorState, StateField, StateEffect, EditorSelection, StateCommand, Prec, Facet, Extension,
        RangeSetBuilder, Text, CharCategory, findClusterBreak, combineConfig} from "../state/index"
...
export class SearchQuery {
     ...
     */
    constructor(config: { search: string, caseSensitive?: boolean, literal?: boolean, regexp?: boolean, replace?: string, wholeWord?: boolean }) {
...
/**
 * Open the search panel if it isn't already open, and move the selection to the first match after
 * the current main selection. Will wrap around to the start of the document when it reaches the end.
 */
